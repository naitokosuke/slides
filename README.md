...
`vp dev` starts one server on <http://localhost:3030>. `/` is the OG-image index of every deck, and each deck is reachable at its date path, e.g. `/2026-02-06/`.
...
`vp run build:slide`, `vp run export:slide` and `vp run create:slide` still use the terminal picker. `build:slide` and `export:slide` also take a date, e.g. `vp run build:slide 2026-02-06`.
